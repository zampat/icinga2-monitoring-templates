...
OBJ="generic-neteye-master"
RES=`icingacli director host exists "$OBJ"`
if [[ $RES =~ "does not exist" ]]
then
   echo "Host '$OBJ' does not exists"
   icingacli director host create --json '
{
    "groups": [
        "neteye servers"
    ...
    "object_name": "generic-neteye-master",
    "object_type": "template",
    "vars": {
        "host_os": "Linux"
    }
   ...
   icingacli director host create --json '
{
    ...
    "master_should_connect": true,
    "object_name": "generic-neteye-satellite",
    "object_type": "template",
    "vars": {
        "host_os": "Linux"
    }
